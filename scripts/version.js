...
(async function () {
  try {
    ...
    if (result) {
      ...
      fs.writeFileSync("next-release-version.txt", result.nextRelease.version);
      fs.writeFileSync("next-release-gitTag.txt", result.nextRelease.gitTag);

      //   move on and skip the rest of the steps
    } else {
